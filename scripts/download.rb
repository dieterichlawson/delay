...
Settings.define :hdp_put, flag: 'h', description: "Put the files on the hdfs", default: false
Settings.resolve!
...
def post_process(year,month,out_dir)
   ...
   if Settings.to_tsv
     ...
     `mv #{filename} #{out_dir}/#{month}-#{year}.tsv`
     filename = "#{out_dir}/#{month}-#{year}.tsv"
   end
   if Settings.concatenate
     ...
     if month == MONTHS.last
       ...
       `hdp-put #{out_dir}/#{year}.tsv /data/raw/#{year}.tsv`
...
if Settings.month.nil? and Settings.year.nil?
  YEARS.each do |year|
  ...
  post_process(Settings.year,Settings.month,Settings.out_dir)
end
